...
public class ModelsGui extends ListGui {
    public ModelsGui(String namespace) {
        super(ItemMods.getTranslationConfig().subTranslation("gui.raw.models"), 4, (s, translation) -> Objects.requireNonNull(ItemMods.getPackManager().getPack(namespace)).getModels()
                .stream().filter(modelAsset -> modelAsset.getName().contains(s)).map(modelAsset -> {
                    var icon = modelAsset.getFallbackTexture();
                    if (icon == null)
                        icon = Material.ARMOR_STAND;
                    return new TranslatedGuiItem(new ItemStackBuilder(icon).displayName(modelAsset.getName()).lore("actions").build()) {{
